use base64::{engine::general_purpose, Engine as _};
use dmzj_proto::details;
use protobuf::Message;
use rsa::{pkcs8::DecodePrivateKey, Pkcs1v15Encrypt, RsaPrivateKey};

const PRIVATE_KEY_STR: &'static str = "";

const MAX_DECRYPT_BLOCK: usize = 128;

fn get_private_key() -> RsaPrivateKey {
    let key_bytes = general_purpose::STANDARD.decode(PRIVATE_KEY_STR).unwrap();

    let key = RsaPrivateKey::from_pkcs8_der(&key_bytes).unwrap();

    key
}

fn decrypt(encrypted: String, key: RsaPrivateKey) -> Vec<u8> {
    let mut result = vec![];
    let encrypted_data = general_purpose::STANDARD.decode(encrypted).unwrap();

    for chunk in encrypted_data.chunks(MAX_DECRYPT_BLOCK) {
        let dec = key.decrypt(Pkcs1v15Encrypt, chunk).unwrap();
        result.extend_from_slice(&dec);
    }

    result
}

#[tokio::main]
async fn main() {
    let res =
        reqwest::get("https://nnv4api.dmzj.com/comic/detail/16187?uid=2665531")
            .await
            .unwrap();

    let text = res.text().await.unwrap();

    let cipher = get_private_key();

    let b = decrypt(text, cipher);

    let r = details::ComicDetailResponse::parse_from_bytes(&b).unwrap();

    println!("{:?}", r.Data.Description);
}
